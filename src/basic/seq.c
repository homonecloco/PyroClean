...
void sequence_set_quality_parameters(Sequence *seq, int quality_offset)
{
    ...
    if (quality_offset == 33) {
        ...
        seq->highest_expected_value = 126;
        seq->highest_raw_read_value = 40;
    } else if (quality_offset == 64) {
        seq->check_quality_values = true;
        seq->lowest_expected_value = -5;
...
int
read_sequence_from_fasta(FILE * fp, Sequence * seq, int max_chunk_length,
					...
					for (i = 0; i < LINE_MAX; i++) {
                        ...
                        if (seq->check_quality_values) {
                            if (seq->qual[q] < seq->lowest_expected_value) {
                                log_and_screen_printf("Warning: Quality for [%s] lower than expected for specified quality offset.\n", seq->name);
                                seq->check_quality_values = false;
                            } else if (seq->qual[q] > seq->highest_expected_value) {
                                log_and_screen_printf("Warning: Quality for [%s] higher than expected for specified quality offset.\n", seq->name);
                                seq->check_quality_values = false;
                            } else if (seq->qual[q] > seq->highest_raw_read_value) {
                                log_and_screen_printf("Warning: Quality for [%s] higher than expected for raw reads.\n", seq->name);
                                seq->check_quality_values = false;
                            }         
                        }
...
int
read_sequence_from_fasta_and_qual(FILE * fp, FILE * fq, Sequence * seq,
				  int max_read_length)
{
 ...
 * 
 * It would be nice to have the "constructur" allocating the pointer.  
 */
...
void
shift_last_kmer_to_start_of_sequence(Sequence * sequence, int length,
				     short kmer_size)
{
...
#endif
	int len = strlen(name) + strlen(seq->name);
	if (len > seq->max_name_length) {
		fprintf(stderr,
...
void sequence_print_fastq_if_qual(FILE * f, Sequence * seq)
{
	assert(seq!=NULL);
	
	int i;
	fprintf(f, "@%s", seq->name);
	for (i = 0; i < seq->length; i++) {
		if(seq->qual[i] > 0 ){
			fprintf(f, "%c", seq->seq[i]);
		}
...
int sequence_count_homopolymer(boolean forward, int pos, Sequence * seq)
{
	...
	signed int i = 0;
	while (seq->seq[pos] == seq->seq[pos + i] && i > 0 && i < seq->length) {
		if (forward)
			i++;
		else
			i--;
...
void sequence_merge_removing_ambiguity(Sequence * seq1, Sequence * seq2){
	...
	if(strcmp(seq1->seq, seq2->seq) != 0){
		fprintf(stderr, "This two sequences were merged but are not equal!: \n");
		sequence_print_fasta(stderr, seq1);
		sequence_print_fasta(stderr, seq2);
	}
...
//Code for the binary representations
